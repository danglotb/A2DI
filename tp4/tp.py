...
newtransition=np.zeros((sizex*sizey,sizex*sizey,4))

for y_s in range(sizey):
	for x_s in range(sizex):
		for y_sp in range(sizey):
			for x_sp in range(sizex):
				for a in range(4):
					transition[y*sizex+x][a]	
